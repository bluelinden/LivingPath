...
def global_Interface(root):
    ...
    gui_foot.pack(
        side="bottom", fill="x", expand=True, anchor="s", pady=(5, 0), padx=(10, 0)
    )
    gui_info.pack(side="left", fill="x", expand=False, anchor="s")
    gui_zone.pack(
        side="top", fill="x", expand=False, anchor="n", padx=(15, 0), pady=(10, 20)
    )
    ctn_para.pack(side="left", fill="y", expand=False, anchor="sw", padx=0, pady=0)
    gui_glob.pack(
        side="bottom", fill=None, expand=False, anchor="sw", padx=(20, 0), pady=(20, 0)
    )
    gui_view.pack(side="top", fill="both", expand=True, anchor="n", pady=0)
    # keep min space width in gui_zone
    ttk.Frame(
        gui_zone, style=frm_style, height=int(0.17 * root.winfo_screenheight()), width=2
    ).grid(rowspan=1000, column=0, row=0, padx=0, pady=0)

    global b_rules, b_paths
    b_rules = gu.Checkbutton(gui_glob, name="display_rules", ini=False)
    s_space = gu.Slider(
        gui_glob, min=-150, max=250, name="letter_spacing", callback=b_rules.select
    )
    b_rules.pack(anchor="w", pady=(20, 20))
    s_space.pack(
        anchor="w", pady=(0, 10), padx=(0, 40)
    )  # padx to prevent gui_para pushing gui_view
    global vecto
    ...
    for i, k in enumerate(gui_font_info):
        ...
        ttk.Label(gui_info, textvariable=gui_font_info[k]).grid(
            column=i, row=0, padx=20, pady=(5, 10)
        )

    refresh_button = gu.ButtonImage(
        root,
        text="refresh",
        size=24,
        hover=True,
    ...
    refresh_butto2 = gu.ButtonImage(
        root, text="text", size=24, hover=True, img_name="text", command=new_wiki
    )
    ...
    img_letter.place(relx=0.5, rely=0.5, anchor="center")
    ...
    wiki.get_wiki_langs()
    try:
        wiki.set_wiki_lang(save_data.readParamFile(0))
    except:
        wiki.set_wiki_lang("en")
    refresh_butto2.update()
    refresh_button.place(
        in_=gui_zone,
        relx=1,
        rely=1,
        x=-30 - 4 - refresh_butto2.winfo_width(),
        y=-50,
        anchor="ne",
    )
...
def open_vecto(*args):
    global vecto
    for widget in vecto.winfo_children():
        widget.pack()
...
def refresh_txt(compute=True):
    if not compute:
        gu.used_glyphs = list(main.font.getGlyphSet().keys())
    ...
    refresh_img(img_txt[0], text_titre(wiki.title, ctn_width, 130))
    ...
    refresh_img(
        img_txt[4], text_block("From wikipedia creative commons", 45, ctn_width)
    )
...
def show_glyph(flag="", char=""):
    ...
    if flag == "prev":
        main.current_glyph = utils.prev_key(glyph_set, main.current_glyph)
    if flag == "next":
        main.current_glyph = utils.next_key(glyph_set, main.current_glyph)

    try:  # input key to glyph name
        saved = main.current_glyph[:]
        if char != "":
            main.current_glyph = font_utils.char_in_font(char, main.font)
    except Exception as e:
        main.current_glyph = saved[:]
    if main.current_glyph == "":
        main.current_glyph = saved[:]
...
def drop(root, e):
    ...
    if ext.lower() == ".lvp":
        save_data.load_data(e.data)


def load_new_font(data):
    if hasattr(root, "config") and data:
        root.config(cursor="watch")
    (
        gui_font_info["name"].set("... Font Loading ..."),
        root.update(),
    )  # _idletasks not working

    main.font.close()
    main.font = TTFont(
        utils.path(data), recalcBBoxes=True, checkChecksums=1, recalcTimestamp=True
    )
    ...
    if "fvar" in main.font:
        ...
        (
            gui_font_info["name"].set("... Convert TTF font to OTF ..."),
            root.update(),
        )  # _idletasks not working
        try:
            ttf2otf.convert(main.font, subroutinize=False)
        except:
            fail = True
    else:
        fail = True
    ...
    main.units = 1 / (
        main.font["head"].unitsPerEm / 1000
    )  # some font are more than 1000 u/em
    main.hbfont = hb.Font(
        hb.Face(hb.Blob.from_file_path(utils.path(data)))
    )  # load metric with uharfbuzz
    ...
    (
        gui_font_info["name"].set("... languages analysys ..."),
        root.update(),
    )  # _idletasks not working
        ...
        try:
            ...
            combo.append(
                (
                    k,
                    re.sub(r"\([^)]*\)", "", l.name),
                    hg_langs[l.part3]["speakers"]
                    if hg_langs[l.part3]["speakers"]
                    else 0,
                    a,
                )
            )
    ...
    menu_items["Language"].add_command(
        label="[ ] supported by font"
    )  # , state='disable' )
    menu_items["Language"].add_separator()
    for lang in combo:
        lab = (
            "[ " + lang[1] + " ]" if lang[3] else "  " + lang[1]
        )  # [] on supported lang
        menu_items["Language"].add_command(
            label=lab, command=partial(set_lang, lang[0])
        )

    main.font_name = str(main.font["name"].getName(16, 3, 1))
    main.font_style = str(main.font["name"].getName(17, 3, 1))
    if main.font_name == "None":
        main.font_name = str(main.font["name"].getName(1, 3, 1))
    if main.font_style == "None":
        main.font_style = str(main.font["name"].getName(2, 3, 1))
    gui_font_info["name"].set(
        "Input font :  " + main.font_name + " - " + main.font_style
    )
...
def show_about_menu():
    gu.show_about_menu(root)


def show_shortcut_menu():
    gu.show_shortcut_menu(root)
...
def setup_menubar():
    ...
    menu_items["File"].add_command(
        label="Import font", command=import_font, accelerator=ctrl + "+N"
    )
    menu_items["File"].add_command(
        label="Export font", command=export_font, accelerator=ctrl + "+E"
    )
    menu_items["File"].add_separator()
    menu_items["File"].add_command(
        label="Save project", command=save_data.dump, accelerator=ctrl + "+S"
    )
    menu_items["File"].add_command(
        label="Open project", command=save_data.load, accelerator=ctrl + "+O"
    )
    menu_items["File"].add_separator()
    menu_items["File"].add_command(
        label="Shortcuts & controls", command=show_shortcut_menu
    )
    if ctrl != "Meta":
        menu_items["File"].add_command(
            label="About LivingPath", command=show_about_menu
        )
    if ctrl == "Meta":
    ...
    for i in range(len(main.plugins)):
        if "diffusion" in main.names[i]:
            menu_items["New layer"].add_command(
                label="reaction-diffusion (experimental)",
                command=partial(main.new_layer, i),
            )
        else:
            menu_items["New layer"].add_command(
                label=main.names[i].replace("_", " "),
                command=partial(main.new_layer, i),
            )
    menu_items["New layer"].add_separator()
    menu_items["New layer"].add_command(
        label="Duplicate layer", command=main.duplicate_layer
    )  # ,accelerator=ctrl+'+V')
    menu_items["New group"].add_command(
        label="New group", command=main.new_group, accelerator=ctrl + "+G"
    )
    menu_items["New group"].add_separator()
    menu_items["New group"].add_command(
        label="Duplicate group",
        command=main.duplicate_group,
...
def selectPath_export(data):  # gui select folder to save font
    ...
    if main.font.flavor == "woff":
        ext = ("Web Open Font", "*.woff")
    if main.font.flavor == "woff2":
    ...
    if file_path:
        ...
        gu.used_glyphs.clear()
        main.process_font_export(
            file_path, name=ffname, style=style, flag=data["combo:Glyph set"]
        )
...
def setup_root(mainRoot):
    ...
    style = ttk.Style()
    style.layout(
        "no_indicatoron.TCheckbutton",  # pathfinders buttons
        [
            (
                "Checkbutton.padding",
                {
