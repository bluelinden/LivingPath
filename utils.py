...
def get_layer_attr(l):
    ...
    for attr in dir(l):
        ...
        if not callable(value) and not (
            attr.startswith(("__", "gui_")) or attr in ("group", "frame", "name", "n")
        ):
...
def new_outline(n_points, n_contours):
    ...
    err = ft.FT_Outline_New(
        library,
        ft.FT_UInt(n_points),
        ft.FT_Int(n_contours),
        ctypes.byref(raw_ft_outline),
    )
    if err:
        raise ft.FT_Exception(err)
    try:
...
LOCK_DIR = os.path.join(os.path.expanduser("~"), ".cache", "LivingPath")
