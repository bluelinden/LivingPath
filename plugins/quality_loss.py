...
class Layer(Plugin):
    ...
    def gui(s, frame):
        ...
        gui.Slider(frame, min=0, max=4, ini=1, layer=s, name="interpolation").pack(
            anchor="nw"
        )
        gui.Slider(frame, min=0, max=10, ini=3, layer=s, name="sharpen").pack(
            anchor="nw", pady=(20, 0)
        )
        gui.Slider(frame, min=0, max=255, ini=127, layer=s, name="threshold").pack(
            anchor="nw"
        )
    ...
    def run(s, img):
        ...
        img = cv2.resize(
            img,
            (
                1 + int(size[0] * s.resolution_width / 1000),
                1 + int(size[1] * s.resolution_height / 1000),
            ),
