    ...
    for x in range(w):
        for y in range(h):
            ...
            laplace_a = (
                ...
                * 0.05
                + (U[x, y - 1] + U[x, y + 1] + U[x - 1, y] + U[x + 1, y]) * 0.2
                - u
            )
            laplace_b = (
                (V[x - 1, y - 1] + V[x + 1, y + 1] + V[x - 1, y + 1] + V[x + 1, y - 1])
                * 0.05
                + (V[x, y - 1] + V[x, y + 1] + V[x - 1, y] + V[x + 1, y]) * 0.2
                - v
            ) * 0.5
            nextImgA[x, y] = min(
                1, max(0, u + laplace_a - uvv + (fk[x, y, 0] * (1 - u)))
            )  # *1.38
            nextImgB[x, y] = min(
                1, max(0, v + laplace_b + uvv - ((fk[x, y, 1] + fk[x, y, 0]) * v))
            )  # *0.43
...
class Layer(Plugin):
    ...
    def gui(s, frame):
        gui.Slider(
            frame, max=0.5, min=0.01, ini=0.1, format="%0.2f", layer=s, name="size"
        ).grid(column=0, row=0, sticky="W")
        gui.Slider(frame, max=200, ini=90, layer=s, name="threshold").grid(
        ...
        gui.Slider(frame, layer=s, min=0, max=3000, ini=1000, name="growing time").grid(
            column=0, row=6, sticky="W"
        )
        gui.Checkbutton(frame, layer=s, name="invert", ini=False).grid(
