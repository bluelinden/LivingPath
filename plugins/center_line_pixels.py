...
class Layer(Plugin):
    ...
    def gui(s, frame):
        ...
        skel.switch()
        opt = gui.Optionbutton(
            frame, layer=s, nbr=6, pos="inline", name="mode", img_name="line"
        )
    ...
    def run(s, img):
        ...
        if s.mode == 0:
            img = skeletonize(img, method="lee")  # 0.23 minimalist
        # if s.mode == 0 : img = skeletonize(img, method='zhang') # 0.12 fastest
        if s.mode == 1:
            img = cv2.ximgproc.thinning(
                img, thinningType=cv2.ximgproc.THINNING_ZHANGSUEN
            )  # 0.15 ressemble au lee
        if s.mode == 2:
            img = cv2.ximgproc.thinning(
                img, thinningType=cv2.ximgproc.THINNING_GUOHALL
            )  # 0.2
        if s.mode == 3:
            img = medial_axis(img)  # 0.15 calld topological skeleton
        if s.mode == 4:
            img = s.cv_custo_skeleton(img, 0)
        if s.mode == 5:
            img = s.cv_custo_skeleton(img, 1)
    ...
    def cv_custo_skeleton(s, img, mode):
        ...
        element = cv2.getStructuringElement(
            k[mode], (s.distortion_width * 2 + 1, s.distortion_height * 2 + 1)
        )
        done = False
        while not done:
