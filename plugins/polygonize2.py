...
class Layer(Plugin):
    ...
    def gui(s, frame):
        gui.Slider(
            frame, layer=s, min=0.01, max=2, name="accuracy", format="%0.2f", ini=0.03
        ).pack(anchor="w")
        setattr(s, "simplify_path", float(0.45))  # we dont need symplify control
        gui.Slider(
            frame, layer=s, max=1.34, name="curves_limit", format="%0.2f", ini=0.0
        ).pack(anchor="w")

        gui.Slider(frame, layer=s, max=200, name="dots_distance", ini=100).pack(
            anchor="w"
        )
    ...
    def run(s, img):
        ...
        if False:
            ...
            )  # some main.font are more than 1000 u/em
            height = (
                main.font["OS/2"].usWinAscent * s
                + main.font["OS/2"].usWinDescent * s
                + 2 * utils.margin
            )
            width = gs[g].width * s + 2 * utils.margin
            img = pen.image(
                width=width, height=height, transform=(s, 0, 0, -s, 0, height)
            )
            ...
            for curve in path:
                ...
                for seg in curve.asSegments():
                    ...
                    for position in range(
                        int(s.dots_distance - rest), int(size), s.dots_distance
                    ):
