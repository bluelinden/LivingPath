...
class Layer(Plugin):
    ...
    def gui(s, frame):
        gui.Slider(frame, max=100, min=1, ini=20, layer=s, name="quantity").grid(
            column=0, row=0, sticky="W"
        )
        gui.Slider(frame, max=400, min=1, ini=50, layer=s, name="size").grid(
            column=0, row=1, sticky="W"
        )
        gui.Slider(frame, max=400, min=1, ini=50, layer=s, name="repultion").grid(
            column=0, row=2, sticky="W"
        )
        gui.Slider(frame, layer=s, min=0, max=1000, ini=100, name="time").grid(
        ...
        gui.Checkbutton(frame, layer=s, name="over", ini=True).grid(
            column=0, row=6, sticky="W"
        )
        gui.Checkbutton(frame, layer=s, name="outline", ini=False).grid(
            column=0, row=7, sticky="W"
        )

        gui.Slider(frame, max=400, min=1, ini=200, layer=s, name="x").grid(
            column=0, row=8, sticky="W"
        )
        gui.Slider(frame, max=200, min=1, ini=50, layer=s, name="y").grid(
            column=0, row=9, sticky="W"
        )
        gui.Slider(frame, max=200, min=1, ini=60, layer=s, name="z").grid(
            column=0, row=10, sticky="W"
        )
        gui.Slider(
            frame, max=2, min=0, ini=0.1, format="%0.2f", layer=s, name="a"
        ).grid(column=0, row=11, sticky="W")
        gui.Slider(
            frame, max=20, min=0, ini=0.9, format="%0.2f", layer=s, name="b"
        ).grid(column=0, row=12, sticky="W")
        gui.Slider(
            frame, max=2, min=0, ini=0.01, format="%0.2f", layer=s, name="c"
        ).grid(column=0, row=13, sticky="W")
        gui.Slider(frame, max=2, min=0, ini=0, format="%0.2f", layer=s, name="d").grid(
            column=0, row=14, sticky="W"
        )
        gui.Slider(frame, max=2, min=0, ini=0, format="%0.2f", layer=s, name="e").grid(
        ...
        for x in range(w // q):
            for y in range(h // q):
                ...
                inertia = pymunk.moment_for_circle(
                    mass, random.randint(115, 350), radius, (0, 0)
                )
    ...
    def generate_geometry(s, img, space, draw):
        ...
        for polyline in line_set:
            ...
            for i in range(len(line) - 1):
                ...
                space.add(s.shape)
            if s.outline:
                draw.line(line, fill=100, width=s.outline_width, joint="curve")
            if s.outline:
                utils.ellipse(
                    s.outline_width, line[0][0], line[0][1], 100, draw
                )  # close draw.line
