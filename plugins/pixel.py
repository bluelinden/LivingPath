...
class Layer(Plugin):
    ...
    def gui(s, frame):
        gui.Slider(frame, max=300, min=1, ini=20, layer=s, name="resolution").grid(
            column=1, row=1, sticky="W"
        )
        gui.Slider(frame, max=100, ini=0, layer=s, name="horizontal_position").grid(
            column=1, row=2, sticky="W"
        )
        gui.Slider(frame, max=100, ini=0, layer=s, name="vertical_position").grid(
