...
class Group:
    ...
    def del_layer(s, n, refresh=True):
        if len(s.layers) != 1 or s.n != 0:  # prevent delete 1st layer
            ...
            for l in range(len(s.layers)):
                s.layers[l].gui_position(l)
            main.over_layer = None

            if refresh:
                if len(s.layers) == 0:
                    main.del_group(s.n)
                if len(s.layers) == 0 and main.layer.group.n == s.n:
                    main.select_layer(main.groups[0].layers[0])
                if main.layer.group.n == s.n and main.layer.n >= s.n:
                    main.select_layer(main.groups[0].layers[0])
                gui.refresh()
            # else :
    ...
    def position(s, n):
        ...
        s.op_frame.grid_remove() if n == 0 else s.op_frame.grid(
            column=(n * 2), row=0, sticky="n", rowspan="80", padx=0, pady=0
        )

    def set_op(s, op, refresh=True):
        s.op = op
        for i in range(5):
            s.gui_op[i]["image"] = s.ico_on[i] if i == op else s.ico_off[i]
        print("SET OP : ", op)
        if refresh:
            gui.refresh()
    ...
    def onEnter(s, i, e):
        s.gui_op[i]["image"] = s.ico_on[i]

    def onLeave(s, i, e):
        if s.op != i:
            s.gui_op[i]["image"] = s.ico_off[i]
...
class Layer(Plugin):
    ...
    def gui(s, frame):
        gui_utils.Checkbutton(frame, layer=s, name="outline", ini=False).pack(
            anchor="w"
        )
        gui_utils.Slider(frame, layer=s, max=200, name="outline_width", ini=100).pack(
            anchor="w"
        )

        s.gui_join = gui_utils.Optionbutton(
            frame,
            layer=s,
            nbr=3,
            ini=2,
        ...
        s.gui_limit_join = gui_utils.Slider(
            frame, layer=s, max=160, name="corner_join_limit", ini=100
        )
    ...
    def set_limit(s, x):
        print(x.var)
        if int(x.var) == 2:
            gui_utils.switch(s.gui_limit_join, flag=True)
        else:
            gui_utils.switch(s.gui_limit_join, flag=False)
