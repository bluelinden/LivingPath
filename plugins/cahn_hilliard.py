...
class Layer(Plugin):
    ...
    def gui(s, frame):
        ...
        gui.Slider(
            frame, layer=s, min=0.5, max=2, ini=1.0, format="%0.3f", name="depth"
        ).pack(anchor="nw")
        gui.Slider(
            frame, layer=s, min=0.0, max=0.5, ini=0.1, format="%0.3f", name="noise"
        ).pack(anchor="nw")
        gui.Slider(
            frame, layer=s, min=1, max=1000, ini=50, name="recomposition", slow=True
        ).pack(anchor="nw")
        # gui.Checkbutton(frame, layer=s, name='invert', ini=False).pack(anchor='nw')
        gui.Slider(frame, min=30, max=230, ini=127, layer=s, name="threshold").pack(
            anchor="nw", pady=(20, 0)
        )
    ...
    def run(s, img):
        if platform.system() == "Windows":  # numpy version
            originSize = img.size
            img = img.resize(
                (int(img.width * s.size), int(img.height * s.size)), Image.NEAREST
            )
            ...
            for i in range(0, Nsteps):
                ...
                dfdc_hat[:] = fft2(
                    np.array(
                        dfdc(np.array(c[i - 1], dtype=np.float32)), dtype=np.float32
                    )
            ...
            originSize = img.size
            img = img.resize(
                (int(1 + img.width * s.size), int(1 + img.height * s.size)),
                Image.LANCZOS,
            )
            ...
            c[0] = torch.tensor(
                img_ini + noise * rng.standard_normal((w, h)),
                dtype=torch.float32,
                device=device,
            )
