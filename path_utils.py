...
def pen_to_img(pen, font, g):  # has to be FreeTypePen
    ...
    )  # some font are more than 1000 u/em (and main unreachable)
    ...
    try:  # garde fou, utile ? ou pas
        img = pen.image(
            ...
            width=width,
            height=height,
            transform=(s, 0, 0, s, m + lsb, font["OS/2"].usWinDescent * s + 2 * m),
        )
    except e:
        return Image.new("L", (500, 1000), 255)
...
def ftoutline_contour(
    outline, layer, gs=None, offset=(1, 0, 0, 1, 0, 0), opened=False, units=1
):
    pen = FreeTypePen(gs)
    ...
    width, linecap, join, limit = (
        layer.outline_width,
        layer.outline_cap if opened else ft.FT_STROKER_LINECAP_BUTT,
        layer.corner_join,
        layer.corner_join_limit * 1000,
    )
    coef = 70  # outline slider coef
    stroker.set(int(width / units * 20), linecap, join, limit)
    if not layer.outline and layer.outline_width > 100:
        stroker.set(int((width - 100) * coef / units), linecap, join, limit)
    if not layer.outline and layer.outline_width <= 100:
        stroker.set(int((100 - width) * coef / units), linecap, join, limit)
...
def pt(a):
    coef = 64
    return (
        utils.constrain(a.x // coef, -4000, 4000),
        utils.constrain(a.y // coef, -4000, 4000),
    )  # constrain aberations
    ...
    for path in vector.paths:
        ...
        for v in path.splines:
            ...
            if v.degree == 3:  # CUBICTYPE
                tpen.curveTo(
                    (v.points[1].x, v.points[1].y),
                    (v.points[2].x, v.points[2].y),
                    (v.points[3].x, v.points[3].y),
                )
        tpen.endPath()
