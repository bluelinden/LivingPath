...
class Slider(ttk.Frame):
    def __init__(
        ...
        if not hasattr(s.layer, s.name):
            s.set_attach_val(ini)  # set ini only at layer init

        s.val = ttk.Label(
            s, text=s.format % s.get_attach_val()
        )  # display fix slider number
    ...
    def set_attach_val(s, val=None):
        if val == None:
            val = s.get()  # if not ini
        if s.format == "%d":
            setattr(s.layer, s.name, int(val))
        if s.format != "%d":
            setattr(s.layer, s.name, float(val))
    ...
    def onclick(
        s, e
    ):  # disable click on slider ( that caused bug with heavy computing load )
        if s.slider.identify(e.x, e.y) == "Horizontal.TickScale.trough":
            return "break"
        else:
            return None
...
class LockSliders(ttk.Frame):
    def __init__(s, context, ini, slider1, slider2):
        ...
        s.b = ttk.Checkbutton(
            s, style="no_indicatoron.TCheckbutton", command=s.toggle, takefocus=False
        )
        gui.root.update()
        s.b.grid(row=0, column=1, rowspan=3, sticky="s", pady=(0, 3))
        s.b.bind("<Enter>", s.onEnter)
        s.b.bind("<Leave>", s.onLeave)
        if hasattr(s.layer, s.name):
            s.lock = not getattr(s.layer, s.name)
        else:
            s.lock = not ini  # set ini only at layer init
        s.toggle()

    def toggle(s):
        s.lock = not s.lock
        if s.lock:
            s.b["image"], s.s1.callback, s.s2.callback = s.ico_on, s.set_s2, s.set_s1
        else:
            s.b["image"], s.s1.callback, s.s2.callback = s.ico_off, None, None
    ...
    def switch(s, flag=None):
        switch(s.s1, s.s2, s.b, flag=flag)

    def onEnter(s, e):
        s.b["image"] = s.ico_off if s.lock else s.ico_on

    def onLeave(s, e):
        s.b["image"] = s.ico_on if s.lock else s.ico_off
...
class ButtonImage(ttk.Button):
    def __init__(
        ...
        **kwargs,
        ...
        if img_name:
            ...
            s.ico_on_alt = (
                get_img(img_name + "-on-alt.png", (size, size))
                if s.flag in ("click", "drag")
                else None
            )
            s.ico_off_alt = (
                get_img(img_name + "-off-alt.png", (size, size))
                if s.flag in ("click", "drag")
                else None
            )

            if hover:
                s.bind("<Enter>", s.enter)
            if hover:
                s.bind("<Leave>", s.leave)
            if not hover:
                s.enter(None)
            if hover:
                s.leave(None)
    ...
    def leave(s, e):
        s["image"] = (
            s.ico_off if s.active else s.ico_off_alt if s.flag == "click" else s.ico_on
        )
        if s.hover:
            s["image"] = s.ico_off
        if s.flag == "drag":
            s["image"] = s.ico_on_alt if s.active else s.ico_off_alt
...
class Optionbutton(ttk.Frame):
    def __init__(
        ...
        if hasattr(s.layer, s.name):
            ini = getattr(s.layer, s.name)  # set ini only at layer init
        s.set(ini, refresh=False)

    def set(s, i, refresh=True):
        s.var = i
        for i in range(len(s.b)):
            s.b[i]["image"] = s.ico_on[i] if i == s.var else s.ico_off[i]
    ...
    def onEnter(s, i, e):
        s.b[i]["image"] = s.ico_on[i]

    def onLeave(s, i, e):
        if s.var != i:
            s.b[i]["image"] = s.ico_off[i]
    ...
    for gui_elem in elems:
        for child in gui_elem.winfo_children():
            if hasattr(child, "state"):
                print(
                    "state", child["state"]
                )  # idk why not working without this print...
                if flag == False:
...
class ScrolledFrame:  # https://github.com/nikospt/tk-ScrolledFrame/
    def __init__(s, parent, side="right", axis="y", speed=15, *args, **kwargs):
        ...
        if axis == "y" or axis == "xy" or axis == "yx" or axis == "both":
            s.yscrollbar = ttk.Scrollbar(
                s.container, orient="vertical", command=s.canvas.yview
            )
        if axis == "x" or axis == "xy" or axis == "yx" or axis == "both":
            s.xscrollbar = ttk.Scrollbar(
                s.container, orient="horizontal", command=s.canvas.xview
            )
    ...
    def updateContent(s):
        ...
        if s.yscrollbar != None:
            ...
            s.canvas.configure(
                yscrollcommand=s.yscrollbar.set,
                scrollregion=s.canvas.bbox("all"),
                yscrollincrement=s.speed,
            )
            if s.canvas.yview() == (0.0, 1.0):  # content fit container
                if s.side == "left":
                    s.yscrollbar.place(width=0, height=0)  # hide
                [s.UnBindMouseWheel(x) for x in [s.content, s.canvas, s.container]]
            else:
                # s.yscrollbar.pack_forget()
                if s.side == "left":
                    s.yscrollbar.place(
                        anchor="nw", width=20, height=s.container.winfo_height()
                    )  # show
        ...
        s.canvas.pack(
            ...
            fill="y",
            expand=True,
            padx=(20, 0) if s.side == "left" else (0, 0),
        )
        s.container.update()
        s.resize()
    ...
    def BindMouseWheel(s, widget):
        if s.root.call("tk", "windowingsystem") == "x11":
            ...
            widget.bind("<MouseWheel>", s.scrollMouseWheel)
    ...
    def scrolldown(s, e):
        s.canvas.yview_scroll(int(-1), "units")  # linux
    ...
    def grid(s, *args, **kwargs):
        s.container.grid(*args, **kwargs)

    def pack(s, *args, **kwargs):
        s.container.pack(*args, **kwargs)
...
class AskBox(object):
    ...
    def __init__(
        s, root, title, data=None, callback=None, btn1="Export", btn2="Cancel"
    ):
...
class LoadBox(object):
    ...
    def __init__(s, root, title):
        ...
        s.txt = tk.StringVar()
        ttk.Label(frame, textvariable=s.txt).pack(
            fill="x", expand=True, padx=0, pady=(0, 0)
        )
...
def show_about_menu(root):
    ...
    top = tk.Toplevel(
        root, borderwidth=0, padx=0, pady=0
    )  # je ne suis pas arrivé à retirer le border 2px
...
def close_splash_screen():
    if (
        ...
        and "_PYI_SPLASH_IPC" in os.environ
