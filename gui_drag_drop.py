    ...
    for g in main.groups:
        for l in g.layers:
            if l.frame is widget2:
                ...
                if row == 0:
                    row += 1  # dont take "outline" layer position
                if last_elem:
                    row = len(main.groups[col].layers)
...
def drag_motion(event, frame, start):
    ...
    if firstDrag:
        ...
        event.widget.bind(
            "<ButtonRelease-1>",
            lambda event: drag_release(event, frame, clone, grid_info),
        )
    firstDrag = False
...
def drag_release(event, frame, clone, grid_info):
    ...
    if target:
        itself = target.master  # to check if target == moved frame
    ...
    if (
        not target_frame
        or target_frame.master is not frame.master
        or target_frame is frame
    ):
        print("not BUTTON")
        last_elem = True
    ...
    if (
        ...
        and target_frame is not frame
    ):  # target is not outline
...
def clone_widget(widget, master=None):
    ...
    if isinstance(cloned, ttk.Frame):
        ttk.Frame(cloned, style="cover.TFrame").grid(
            row=0, column=0, rowspan=1, columnspan=4, sticky="nsew"
        )
