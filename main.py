...
def time(msg):
    ...
    if msg == "end":
        ...
        print(
            msg + ":", str(perf_counter() - last_time).replace("0", "-")[0:5], end="  "
        )
...
def get_current_img(key, compute=True):
        ...
        for g in groups:  # apply algos to pixels
            if g.layers[0].active:  # if group is active
                ...
                for l in g.layers:
                    if l.active:
                        out = l.run(out)
                        if key is current_glyph:
                            l.save(out)  # dont save if txt preview
                if g.n > 0:
                    out = operator_img(out, prev_img, g.op)
            else:
                if g.n > 0:
                    out = prev_img
                if g.n == 0:
                    ...
                    out = PIL.Image.new(
                        "L", (blk.width, blk.height), 255
                    )  # if group is not active
            prev_img = out
        # time("algo")
        if key is current_glyph:
            img = out  # save img if not txt preview
    else:
        out = img
    ...
    # time("end")
    if (
        gui.visual_info and over_layer and over_layer.img and over_layer.active
    ):  # show blue gost glyph
...
def text_to_font(txt, out_font, char_to_glyph=True, title="Progress", box=True):
    ...
    if box and len(txt) > 0:
        box = gui_utils.LoadBox(gui.root, title)
    for i, key in enumerate(txt):
        if box:
            if box.stop or globals()["stop_process"]:
                globals()["stop_process"] = True
                break
...
def process_font_export(path="", name=None, style=None, flag="all"):
    ...
    if "all" in flag:  # filter GLYPH SETS
        ...
        glyphs = ""
        if "punctuation" in flag:
            flag = flag.replace("punctuation", "") + string.punctuation
        if "digits" in flag:
            flag = flag.replace("digits", "") + string.digits
        if "uppercase" in flag:
            flag = flag.replace("uppercase", "") + string.ascii_uppercase
        if "lowercase" in flag:
            flag = flag.replace("lowercase", "") + string.ascii_lowercase
        if "basic latin" in flag:
            flag = (
                flag.replace("basic latin", "")
...
def select_layer(selected):
    global layer
    layer = selected
    for child in gui.gui_para.content.winfo_children():
        child.destroy()
    for grp in groups:
...
def del_group(grp, select_last=True):
    if grp.n != 0:
        ...
        g = groups[grp.n]
        for l in reversed(g.layers):
            g.del_layer(l.n, False)
        if g.op_frame:
            g.op_frame.destroy()
        if g.drag_frame:
            g.drag_frame.destroy()
        del groups[grp.n]
        for i in range(len(groups)):
            groups[i].position(i)
        if select_last:
            if layer.group.n == grp.n:
                select_layer(groups[0].layers[-1])
        gui.refresh()
...
def duplicate_layer(g=None, refresh=True):
    if layer.n != 0:  # do mnothing if selected layer is group path
        ...
        print("###", layer.active)
        if not layer.active:
            layer.toggle(refresh=False, unify=True)
        layer.gui(gui.gui_para.content)
        select_layer(layer)
        if refresh:
            gui.refresh()
...
def duplicate_group():
    g_old = layer.group
    new_group()
    for name, val in utils.get_layer_attr(g_old.layers[0]):
        setattr(layer, name, val)  # setup outline param
...
def runApp():
    ...
    try:
        gui.load_new_font(utils.path(save_data.readParamFile(1)))
    except Exception as e:
        gui.load_new_font(utils.path("files/1.otf"))
    gui.show_glyph("next")
    gui.show_glyph("prev")  # regularize current glyph if not in font
