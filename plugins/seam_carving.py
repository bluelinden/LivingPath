...
class Layer(Plugin):
    ...
    def gui(s, frame):
        ...
        gui.Slider(
            frame, min=1, max=100, ini=30, layer=s, name="resolution", slow=True
        ).pack(anchor="nw")

        gui.Checkbutton(frame, layer=s, name="preserve_shape", ini=False).pack(
            anchor="nw"
        )
        gui.Checkbutton(frame, layer=s, name="reverse_gravity", ini=False).pack(
            anchor="nw"
        )

        gui.Slider(
            frame, min=1, max=5, ini=1.8, layer=s, format="%0.2f", name="zoom"
        ).pack(anchor="nw")
        gui.Slider(
            frame,
            min=-1,
    ...
    def run(s, img):
        if not s.reverse_gravity:
            img = img.transpose(Image.Transpose.ROTATE_180)

        w, h = img.size[0], img.size[1]
        img = img.resize(
            (
                int(utils.mapping(s.resolution, 1, 100, 1, w / 5)),
        ...
        resize = (
            int(utils.mapping(s.stretch_width, 0, 100, img.width, 2)),
            int(utils.mapping(s.stretch_height, 0, 100, img.height, 2)),
        )

        imgA = seam_carving.resize(
            imgA, size=resize, energy_mode="forward", order="width-first"
        )

        resize = (
            int(1.5 * resize[0] + s.correct_ratio * resize[0]),
        ...
        imgA = seam_carving.resize(
            imgA,
            size=img.size,
            energy_mode="forward" if s.preserve_shape else "backward",
            order="width-first",
        )
        ...
        img2.paste(
            img, ((img2.width - img.width) // 2, (img2.height - img.height) // 2)
        )
