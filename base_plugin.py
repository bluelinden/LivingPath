...
class Plugin(object):
    ...
    def setup_gui(s):
        ...
        s.gui_drag = gui_utils.ButtonImage(
            s.frame, size=h, img_name="drag", flag="drag"
        )
        s.gui_del = gui_utils.ButtonImage(s.frame, size=h, img_name="del")
        s.gui_toggle = gui_utils.ButtonImage(
            s.frame, size=h, img_name="toggle", command=s.toggle, flag="click"
        )
        ...
        if s.name != "outline":
            ...
            s.gui_button.grid(
                column=1,
                row=0,
                padx=(s.gui_button.winfo_height() + 4 + 2, 2),
                pady=0,
                sticky="w",
            )  # add padding to the 1st
    ...
    def toggle(s, refresh=True, unify=False):
        ...
        if s.name == "outline":
            ...
            if (
                s.active and not s.group.layers[0].active
            ):  # activate group if group is disabled
    ...
    def gui_position(s, n, group=None):
        ...
        if s.name != "outline":
            s.gui_del.setCommand(partial(s.group.del_layer, s.n))
        if s.name == "outline":
            s.gui_del.setCommand(partial(main.del_group, s.group))
    ...
    def set_main_frame(s):
        ...
        s.frame.grid(
            column=(s.group.n * 2 + 1), row=s.n, padx=10, pady=2, sticky="nsew"
        )
    ...
    def gui_img(img):
        img_min = ImageTk.PhotoImage(
            img.crop((150, 150, img.width - 150, img.height - 150)).resize((20, 20), 1)
        )
    ...
    def gost(s):
        ...
        if s.ref_plugin == -1:
            b = object.__new__(group.Layer)
        if s.ref_plugin != -1:
            b = object.__new__(main.plugins[s.ref_plugin].Layer)
