...
def glyph_to_font_outline(g, in_font, font, group):
    ...
    if "CFF " in font:
        pen = T2CharStringPen(gs[g].width, font.getGlyphSet())
    tpen = TransformPen(pen, (1, 0, 0, 1, 0, 0))

    stroker = ft.Stroker()
    width, linecap, join, limit = (
        l.outline_width,
        ft.FT_STROKER_LINECAP_BUTT,
        l.angle_join,
        l.angle_join_limit * 1000,
    )
    stroker.set(width * 20, linecap, join, limit)
    if not l.outline and width > 100:
        stroker.set((width - 100) * 20, linecap, join, limit)
    if not l.outline and width <= 100:
        stroker.set((100 - width) * 20, linecap, join, limit)
    ...
    with utils.new_outline(n_points, n_contours) as stroked_outline:
        ...
        if not l.outline and l.outline_width > 100:
            stroker.export_border(outline.get_outside_border(), stroked_outline)
        if not l.outline and l.outline_width <= 100:
            stroker.export_border(outline.get_inside_border(), stroked_outline)
        stroked_outline.decompose(
            tpen,
            move_to=move_to,
            line_to=line_to,
            conic_to=conic_to,
    ...
    if "glyf" in font:
        font["glyf"][g] = pen.glyph(dropImpliedOnCurves=False)
    if "CFF " in font:
        font["CFF "].cff.topDictIndex[0].CharStrings[g] = pen.getCharString()
...
def vectorization(img, params=params):
    ...
    path = bmp.trace(
        alphamax=params.curves_limit, opticurve=True, opttolerance=params.simplify_path
    )  # turdsize=params.minimal_zone
...
def path_to_font(path, glyph, font, path_type="potrace"):
    if not params.letter_spacing == 0:
        font["hmtx"][glyph] = (
            int(font["hmtx"][glyph][0] + params.letter_spacing),
            font["hmtx"][glyph][1],
        )
    gs = font.getGlyphSet()
    ...
    if "CFF " in font:
        X = (
            font["CFF "].cff[0].Private.nominalWidthX
            if "nominalWidthX" in font["CFF "].cff[0].Private.rawDict
            else 0
        )
        pen = T2CharStringPen(
            gs[glyph].width - X, gs
        )  # gliphWidth - nominalWidthX (does -X is usefull ?)
    ...
    tpen = TransformPen(
        ...
        (
            ...
            0,
            -s,
            -utils.margin * s + path_utils.get_lsb(gs, glyph) * s,
            font["OS/2"].usWinAscent,
        ),
    )
    if path_type == "potrace":
        path_to_pen(path, tpen)
    if path_type == "array":
        array_to_pen(path, tpen)

    # pprint.pprint(vars(gs[glyph]))
    if "glyf" in font:
        font["glyf"][glyph] = pen.glyph(dropImpliedOnCurves=True)
    if "CFF " in font:
    ...
    for curve in path:
        ...
        for segment in curve:
            ...
            if segment.is_corner:
                ...
                tpen.curveTo(
                    (segment.c1.x, segment.c1.y), (segment.c2.x, segment.c2.y), (x, y)
                )
        tpen.closePath()
...
def text_to_img_HB(text, ttfont, hbfont):
    ...
    typo_ascender, typo_descender = (
        ttfont["OS/2"].usWinAscent,
        ttfont["OS/2"].usWinDescent,
    )
    if "hhea" in ttfont:
        h_ascender, h_descender = ttfont["hhea"].ascent, ttfont["hhea"].descent
    if "vhea" in ttfont:
        v_ascender, v_descender = ttfont["vhea"].ascent, ttfont["vhea"].descent
    contain = False
    direction = "ltr"
    ...
    if buf.glyph_positions != None:
        for info, pos in zip(buf.glyph_infos, buf.glyph_positions):
            ...
            transformed = TransformPen(
                pen, (1, 0, 0, 1, x + pos.x_offset, y + pos.y_offset)
            )
...
def operator_img(img, img2, op):
    if op == 0:
        ...
        img = ImageChops.add(ImageOps.invert(img), img2)
    elif op == 2:
        img = ImageChops.add(img, ImageOps.invert(img2))
    elif op == 3:
        img = ImageChops.lighter(img, img2)
    elif op == 4:
        img = ImageOps.invert(ImageChops.difference(img, img2))
    return img
...
def draw_rules(img, g, font):  # draw visual beziers with PIL
    ...
    if hasattr(font["OS/2"], "usWinAscent"):
        asc = font["OS/2"].usWinAscent / s
    elif hasattr(font["hhea"], "ascender"):
        asc = font["hhea"].ascender / s
    else:
        asc = 4000

    if hasattr(font["OS/2"], "usWinDescent"):
        des = font["OS/2"].usWinDescent / s
    elif hasattr(font["OS/2"], "sTypoDescender"):
        des = abs(font["OS/2"].sTypoDescender) / s
    elif hasattr(font["hhea"], "descender"):
        des = abs(font["hhea"].descender) / s
    else:
        des = 4000

    if hasattr(font["OS/2"], "sCapHeight"):
        cap = font["OS/2"].sCapHeight / s
    elif hasattr(font["OS/2"], "sTypoAscender"):
        cap = abs(font["OS/2"].sTypoAscender) / s
    else:
        cap = 4000

    if hasattr(font["OS/2"], "sxHeight"):
        xheight = font["OS/2"].sxHeight / s
    else:
        xheight = 4000
    ...
    lineLabel(
        draw,
        (width + m, m - mm, width + m, img.height - 70),
        c,
        2,
        str(int(width * s)),
        vert=img.height,
        ...
        for curve in path:  # draw beziers
            ...
            for segment in curve:
                ...
                if not segment.is_corner:
                    draw.line(
                        (segment.c1.x, segment.c1.y, last.x, last.y), bleu2, width=2
                    )
...
def rename_font(font, name, style):
    ...
    for record in font["name"].names:
        ...
        if id == 2:
            record.string = (
                style.encode(enc) if regular_style else "Regular".encode(enc)
            )
        if id == 3 or id == 5:
            record.string += str("; LivingPath 1.0 - " + name_style).encode(
                enc
            )  # unique id, version id
        if id == 4:
            record.string = name_style.encode(enc)  # full name
        if id == 6:
            record.string = (
                str(name + "-" + style).replace(" ", "").encode(enc)
            )  # PostScript name
        # if id == 9:  # Authors names
        # if id == 12:  # Autors URLs Links
        if id == 1 and not regular_style:
            record.string = name_style.encode(enc)
        if id == 16 and not regular_style:
            record.string = name.encode(enc)
        if id == 17 and not regular_style:
            record.string = style.encode(enc)
        if id == 16 and regular_style:
            font["name"].names.remove(record)
        if id == 17 and regular_style:
            font["name"].names.remove(record)
    ...
    if "CFF " in font:
        try:
            if "FamilyName" in font["CFF "].cff[0].rawDict:
                font["CFF "].cff[0].FamilyName = name
            if "FullName" in font["CFF "].cff[0].rawDict:
                font["CFF "].cff[0].FullName = name_style
            if "Weight" in font["CFF "].cff[0].rawDict:
                font["CFF "].cff[0].Weight = style
