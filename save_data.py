...
# Use writable user data directory instead of installation directory
DATA_DIR = os.path.join(os.path.expanduser("~"), ".config", "LivingPath")
os.makedirs(DATA_DIR, exist_ok=True)
PARAMS_FILE = os.path.join(DATA_DIR, "global_params.txt")
...
def writeParamFile(data, line=-1):
    try:
        ...
        with open(PARAMS_FILE, "wb") as f:
            pickle.dump(["en", "files/1.otf"], f, pickle.HIGHEST_PROTOCOL)
...
def readParamFile(line=-1):
    try:
        ...
        # Initialize with defaults on first run
        writeParamFile(["en", "files/1.otf"])
        if line != -1:
            return ["en", "files/1.otf"][line]
        return ["en", "files/1.otf"]


# ------------------------------------------------------------------------------------------#
def dump():
    filename = fd.asksaveasfilename(
        title="Save LivingPath file",
...
def dumpFile(filename):
    with open(filename, "wb") as f:
        ...
        pickle.dump(
            {"groups": data, "params": params, "font file": fontPath},
            f,
...
def load_data(filename):
    with open(filename, "rb") as f:
        ...
        for data_group in data["groups"]:  # build loaded groups
            ...
            g = main.groups[-1]
            for name, val in data_group["layers"][0][
                "params"
            ]:  # set contour layer params
            ...
            for data_layer in data_group["layers"][1:]:  # build layers
                ...
                l.change_order(
                    g.n, data_layer["n"], refresh=False
                )  # set layer position

            g.set_op(data_group["op"], refresh=False)  # set group op

        main.select_layer(main.groups[-1].layers[-1])
        if utils.path_name(data["font file"]) != utils.path_name(
            main.font.reader.file.name
        ):
            try:
