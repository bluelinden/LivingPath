...
class Layer(Plugin):
    ...
    def gui(s, frame):
        ...
        s.corner_join = 2
        gui.Slider(frame, layer=s, min=1, max=300, name="outline_width", ini=110).pack(
            anchor="w"
        )
        ...
        gui.Slider(
            ...
            ini=0.2,
            format="%0.2f",
            name="algo_resolution",
            slow=True,
        ).pack(anchor="w", pady=(25, 0))
        gui.Slider(
            frame, layer=s, max=20, min=0.2, ini=1, format="%0.1f", name="smooth"
        ).pack(anchor="w")
        gui.Slider(
            frame, layer=s, max=20, min=0, ini=1, format="%0.1f", name="smooth2"
        ).pack(anchor="w")
        gui.Slider(
            frame,
            layer=s,
        ...
        gui.Slider(frame, layer=s, max=180, name="corner_threshold", ini=100).pack(
            anchor="w"
        )

        gui.Slider(frame, layer=s, max=2, name="add_i_dot", ini=1, format="%0.1f").pack(
            anchor="w", pady=(25, 0)
        )
    ...
    def run(s, img):
        ...
        img = img.resize(
            (int(img.width * s.algo_resolution), int(img.height * s.algo_resolution)),
            Image.NEAREST,
        )
        w = img.width
        h = img.height
        ...
        m = utils.margin / u
        outline = path_utils.autotrace_to_ftoutline(
            vector,
            (
                1 / s.algo_resolution / u,
                0,
                0,
                1 / s.algo_resolution / u,
                -m,
        ...
        pen = path_utils.ftoutline_contour(
            outline, s, main.font.getGlyphSet(), opened=True, units=main.units
        )
    ...
    def detect_points(s, img):
        ...
        for i in keypoints:
            ...
            cv2.circle(
                img, (int(i.pt[0]), int(i.pt[1])), int(r * 1) + 5, 255, -1
            )  # cache white circle
            cv2.ellipse(
                img,
                (int(i.pt[0]), int(i.pt[1])),
                (int(r * 0.5), r),
                s.i_dot_angle,
                0,
                360,
