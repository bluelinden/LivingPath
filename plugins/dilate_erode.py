...
class Layer(Plugin):
    ...
    def gui(s, frame):
        gui.Slider(frame, max=200, ini=100, layer=s, name="contour_val").grid(
            column=1, row=1, sticky="W"
        )
        gui.Slider(frame, min=2, max=20, ini=5, layer=s, name="kernel_size").grid(
    ...
    def run(s, img):
        ...
        kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (s.kernel_size, s.kernel_size)
        )
